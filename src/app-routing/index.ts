...
function addDeclarationToNgModule(options: any) {
    return (host: any) => {
        ...
        ast_utils_1.addImportToModule(source, modulePath, core_1.strings.classify(`${options.name}Module`), relativePath);
        const nodes: any[] = ast_utils_1.findNodes(source, ts.SyntaxKind.Identifier);
        const call = nodes.filter(n => n.text === 'forRoot' || n.text === 'forChild').pop();
        const routes = call.parent.parent.arguments[0];
        const definition = ast_utils_1.findNodes(source, ts.SyntaxKind.Identifier).filter((s: any) => s.text === routes.text).shift();
...
export function default_1(options: any) {
    return (host: any) => {
        if (!options.project) {
        ...
        const componentTemplateSource = schematics_1.apply(schematics_1.url('./files'), [
            options.spec ? schematics_1.noop() : schematics_1.filter((path: any) => !path.endsWith('.spec.ts')),
            options.inlineStyle ? schematics_1.filter((path: any) => !path.endsWith('.__styleext__')) : schematics_1.noop(),
            options.inlineTemplate ? schematics_1.filter((path: any) => !path.endsWith('.html')) : schematics_1.noop(),
